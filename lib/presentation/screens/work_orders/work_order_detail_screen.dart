// lib/presentation/screens/work_orders/work_order_detail_screen.dart
...
class WorkOrderDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Órden de Trabajo #$orderId'),
        leading: IconButton(
      ...
      body: orderAsync.when(
        ...
        data: (order) {
          final isCancelled = order.status == WorkOrderStatus.cancelled;
     ...
     style: const TextStyle(color: AppColors.textSecondary, fontSize: 13))
                ...
                // ── Servicios Incluidos ──────────────────────
                _SectionCard(
                  title: 'Servicios / Trabajos (${order.numItems})',
                  child: Column(
...
class _OrderProgressBar extends StatelessWidget {
  final List<WorkOrderStatus> steps;
              ...
              if (idx < steps.length - 1)
                Expanded(
                  child: Container(
                    ...
                    color:
                        idx < currentStep ? AppColors.accent : AppColors.border,
                  ),
                ),
            ],
          ),
